...
export class MailservicesController {
  ...
  async sendContactUsEmail(@Body() contactUsDto: ContactUsDto) {
    ...
    try {
      const result = await this.mailService.sendContactUsEmail(name, email, message, phone, subject);
      if (result.success) {
  ...
  async sendFeedbackEmail(@Body() feedbackDto: FeedbackDto) {
    ...
    try {
      const result = await this.mailService.sendFeedbackEmail(name, email, feedback);
      if (result.success) {
        return { success: true, message: result.message };
      } else {
        throw new HttpException(result.message, HttpStatus.INTERNAL_SERVER_ERROR);
